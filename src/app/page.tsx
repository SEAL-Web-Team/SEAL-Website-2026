  ...
  return (
    <div className="flex flex-col">
      ...
      <section className="relative h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="relative text-center px-6 max-w-4xl w-full">
          ...
          <img
            ...
            className="h-10 w-auto mx-auto mb-8 brightness-0 invert opacity-70"
          />
          ...
          <div className="flex items-center justify-center gap-4 flex-wrap">
            <Link
              href="/projects"
              className="px-7 py-3.5 text-sm font-semibold text-white bg-white/10 border border-white/20 rounded-lg hover:bg-white/20 transition-all backdrop-blur-sm"
            >
              Our Research
            </Link>
            <Link
      ...
      <section className="py-28 px-8 border-b border-white/[0.06]">
        <div className="max-w-6xl mx-auto">
          <div className="grid lg:grid-cols-[1fr_2fr] gap-16 items-start">
            <div>
              <p className="text-xs uppercase tracking-widest text-purple-400 mb-4">About</p>
      ...
      <section className="py-28 px-8 border-b border-white/[0.06]">
        <div className="max-w-6xl mx-auto">
          <div className="flex items-baseline justify-between mb-16 flex-wrap gap-4">
            <div>
              <p className="text-xs uppercase tracking-widest text-purple-400 mb-4">Capabilities</p>
              <h2 className="text-4xl font-bold text-white">SEAL Teams</h2>
            </div>
            <Link href="/projects" className="text-sm text-slate-500 hover:text-slate-300 transition-colors">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {teams.map((team) => (
              <div
                key={team.id}
                className="relative group p-6 rounded-2xl bg-white/[0.03] border border-white/[0.06] hover:border-white/[0.12] hover:bg-white/[0.05] transition-all duration-200 overflow-hidden cursor-default"
              >
                <div className={`absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r ${team.color.replace("bg-", "from-")} to-transparent`} />
                <div className="flex items-start justify-between mb-5">
                  <span className="text-xs font-mono text-slate-600">{team.id}</span>
                  <div className={`w-2 h-2 rounded-full ${team.color} opacity-80`} />
                </div>
                <p className={`text-[11px] font-semibold uppercase tracking-widest ${team.text} mb-2 opacity-80`}>{team.name}</p>
      ...
      <section className="py-28 px-8">
        <div className="max-w-6xl mx-auto">
          <div className="mb-16">
            <p className="text-xs uppercase tracking-widest text-purple-400 mb-4">Students</p>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {benefits.map((b) => (
              ...
              >
                ...
                <p className={`text-[11px] uppercase tracking-widest ${b.tag} mb-4`}>{b.label}</p>
                <h3 className="text-white font-semibold text-base mb-3 leading-snug">{b.heading}</h3>
                <p className="text-slate-400 text-sm leading-relaxed flex-1 mb-6">{b.description}</p>
                <Link
                  href={b.href}
                  className={`self-start text-sm font-semibold bg-gradient-to-r ${b.accent} bg-clip-text text-transparent hover:opacity-80 transition-opacity`}
                >
                  Learn more →
                </Link>
              </div>
