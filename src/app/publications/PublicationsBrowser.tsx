...
function PubRow({ pub, index }: { pub: Publication; index: number }) {
  return (
    <div className="flex flex-wrap gap-x-5 gap-y-2 py-5 border-b border-white/[0.06] last:border-b-0">
      {/* Index */}
      <div className="w-8 shrink-0 text-right text-slate-500 text-sm tabular-nums pt-0.5 select-none">
        {index}
      </div>
      ...
      <div className="flex-1 min-w-0" style={{ minWidth: "min(100%, 16rem)" }}>
        {pub.url ? (
          <a
            ...
            className="text-[#f1f3f9] font-medium text-[0.9375rem] leading-snug hover:text-[#ab9ffa] transition-colors duration-150 break-words"
          >
            {pub.title}
          </a>
        ) : (
          <span className="text-[#f1f3f9] font-medium text-[0.9375rem] leading-snug break-words">
            {pub.title}
          </span>
        )}
        ...
        <div className="flex flex-row sm:flex-col items-start sm:items-end gap-1.5 shrink-0 pt-0.5 pl-8 sm:pl-0 w-full sm:w-auto">
          {pub.bibtex && (
            <a
              href={pub.bibtex}
              download
              className="text-xs font-medium text-slate-400 bg-white/[0.04] border border-white/[0.08] rounded-md px-2.5 py-1 hover:text-[#f1f3f9] hover:border-white/20 hover:bg-white/[0.07] transition-all duration-150 whitespace-nowrap"
            >
              BibTeX
            </a>
          )}
...
function SectionHeader({
  ...
  return (
    ...
    >
      <h2 className="text-[#f1f3f9] text-xl font-semibold group-hover:text-[#ab9ffa] transition-colors">{title}</h2>
...
export function PublicationsBrowser({ publications }: { publications: PublicationData }) {
      ...
      <div className="page-container-tight">
        ...
        <div className="mb-10 sm:mb-14">
          <input
            ...
            className="w-full bg-white/[0.04] border border-white/[0.08] rounded-lg px-4 py-2.5 text-[#f1f3f9] text-sm placeholder-slate-500 outline-none focus:border-white/20"
          />
        </div>
